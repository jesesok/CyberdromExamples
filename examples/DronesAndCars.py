...
p_2 = Pioneer(method=2, pioneer_ip="127.0.0.2", pioneer_mavlink_port=8005, logger=False)

e_1.go_to_local_point(5, 5)

while True:
    if e_1.point_reached():
        Thread(target=e_2.go_to_local_point, args=[10, 1]).start()
        Thread(target=e_3.go_to_local_point, args=[7.3, 4]).start()

    if e_2.point_reached():
        p_1.arm()
        p_1.takeoff()
        p_1.led_custom(mode=2, timer=10, color1=[127, 12, 41], color2=[55, 171, 42])
        p_1.go_to_local_point(4, 9, 1.5)
    ...
    if p_1.point_reached():
        ...
        p_2.arm()
        p_2.takeoff()
        p_2.go_to_local_point(5.0, 0, 1.5)
